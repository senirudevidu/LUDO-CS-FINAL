...
typedef struct
{
    ...
    int homeStraight;           // Remaining steps for home in homestraight
    int isSick;                 // if sick = 1 if Energized 2 else 0
    int isSickNumber;           // n: goes n rounds with sick or energized
    int mysteryCell;
    int block;
...
{
    char colour;                // 'r' for red, 'g' for green, 'b' for blue, 'y' for yellow
